  ...
  <div>
    ...
    <div class="card-group mx-5">
      <div class="card btn btn-outline-dark" v-for="item in inventory.slice(0, 3)" :key="item._id">
        <router-link :to="`/gift/detail/${item._id}`">
        ...
        <div class="card-body">
          ...
          <p class="card-text"><small class="text-muted" > {{ item.amount - item.consume.length }}
              Remaining</small></p>
          <button v-if="item.amount - item.consume.length > 0" class="btn btn-primary"
